...
const QuizSettingItem: React.FC<QuizSettingItemProps> = ({ setting, title, onChange }) => {
  ...
  return (
    <div className={`rounded-xl border-2 p-3 transition-all ${setting.isSelected ? 'border-blue-400 bg-blue-50' : 'border-gray-200 bg-white'}`}>
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className="text-sm font-semibold text-gray-800">{title}</p>
          <p className="text-xs text-gray-400">Total: {setting.total} items</p>
        </div>
        <button
          onClick={handleToggle}
          className={`w-12 h-6 rounded-full transition-colors relative ${setting.isSelected ? 'bg-blue-500' : 'bg-gray-300'}`}
        >
          <span
...
const QuizSettingModal: React.FC<QuizSettingModalProps> = ({ onClose }) => {
  ...
  // Build initial setting list from stores
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50">
      <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-md max-h-[85vh] flex flex-col shadow-2xl">
        {/* Handle bar (mobile) */}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {settings.length === 0 ? (
            <p className="text-center text-gray-400 py-8">No word lists available. Go back to Home.</p>
          ) : (
            settings.map((s, i) => (
              <QuizSettingItem
                key={s.type}
